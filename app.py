...
# ==============================================================================
# 2. CAPA DE DISEÑO VISUAL "ELITE SUPREMACÍA" (ESTRUCTURA ORIGINAL)
# ==============================================================================
st.markdown("""
    <style>
    /* Ocultar elementos para privacidad y estética */
...
if 'chat_log' not in st.session_state:
    st.session_state.chat_log = [{"role": "assistant", "content": "¡V31 Activa, jefe! Módulo IA Real conectado y listo. 🫡"}]

# ==============================================================================
# 4. MOTORES DE AUDITORÍA (EXTRACTOR COMPLETO)
# ==============================================================================
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if info:
                    ...
                    fallos.append({"Objetivo": url, "Motivo": "Privado/Bloqueado"})
        except Exception as e:
            fallos.append({"Objetivo": url, "Motivo": f"Error: {str(e)[:20]}"})
...
# ==============================================================================
# 6. DESPLIEGUE DE MÓDULOS (MANTENIENDO TODA LA LÓGICA ORIGINAL)
# ==============================================================================
...
# ==============================================================================
# 7. PARTNER IA (CEREBRO GEMINI INTEGRADO)
# ==============================================================================
elif menu == "🤖 PARTNER IA":
    st.markdown("### 🤖 Partner IA (Cerebro Gemini Real)")
    ...
    if prompt := st.chat_input("Escribe tu consulta sobre los datos..."):
        ...
        with st.chat_message("assistant"):
            # Preparar contexto real de los datos actuales
            data_context = "No hay datos cargados."
            if not st.session_state.db_final.empty:
                resumen_vistas = st.session_state.db_final['Vistas'].sum()
                data_context = f"Tabla de datos actual: {st.session_state.db_final.to_string()}. Total vistas: {resumen_vistas}."
            
            try:
                # LLAMADA A LA IA REAL
                full_query = f"Eres el analista experto de BS LATAM. Contexto de los datos: {data_context}. Pregunta del usuario: {prompt}"
                response = model_ia.generate_content(full_query)
                st.markdown(response.text)
                st.session_state.chat_log.append({"role": "assistant", "content": response.text})
            except Exception as e:
                st.error(f"Error en el cerebro IA: {e}")

# ==============================================================================
# 8. SEARCH PRO (MODO MASIVO/BULK INTEGRADO)
# ==============================================================================
elif menu == "🛰️ SEARCH PRO":
    st.subheader("🛰️ Buscador Inteligente Masivo (Bulk Scan)")
    
    # Cambio solicitado: De link único a área de texto para múltiples enlaces
    target_bulk = st.text_area("Pega varios links de Canales o @usuarios (uno por línea):", height=150, placeholder="https://www.tiktok.com/@user1\n@user2")
    
    col_u2 = st.columns([1])[0]
    vistas_min = col_u2.number_input("Vistas mínimas para filtrar", value=60000)
    
    st.divider()
    st.markdown("#### 📅 Rango de tiempo para Escaneo Masivo")
    ...
    if st.button("🚀 INICIAR ESCANEO DE CANALES"):
        # Extraer todos los canales válidos del texto pegado
        canales = re.findall(r"(@[\w\.]+|https?://www\.tiktok\.com/@[^/\s\?]+)", target_bulk)
        
        if canales:
            all_valid_links = []
            canales_con_error = []
            
            with st.status("🛠️ Ejecutando Rastro Masivo...", expanded=True) as status_bulk:
                for canal in canales:
                    clean_user = canal.split('?')[0].rstrip('/')
                    if not clean_user.startswith('http'):
                        clean_user = f"https://www.tiktok.com/@{clean_user.replace('@', '')}"
                    
                    status_bulk.write(f"🔍 Escaneando Objetivo: {clean_user}")
                            ...
                            if res and 'entries' in res:
                                ...
                                for entry in res['entries']:
                                    ...
                                    if v_ts and f_inicio_ts <= v_ts <= f_fin_ts:
                                        link_v = entry.get('url') or f"https://www.tiktok.com/video/{entry.get('id')}"
                                        all_valid_links.append(link_v)
                            else:
                                canales_con_error.append({"Objetivo": clean_user, "Motivo": "Sin acceso/Canal vacío"})
                    except:
                        canales_con_error.append({"Objetivo": clean_user, "Motivo": "Error de conexión (Bypass fallido)"})
                
                if all_valid_links:
                    status_bulk.write(f"✅ Detectados {len(all_valid_links)} videos totales. Iniciando auditoría de métricas...")
                    # Procesar todos los videos encontrados
                    df_exitos, df_fallos_motor = motor_auditor_universal_v24(all_valid_links)
                    st.session_state.db_final = df_exitos
                    # Guardar fallos de canales + fallos del motor
                    st.session_state.db_fallidos = pd.concat([pd.DataFrame(canales_con_error), df_fallos_motor])
                    st.rerun()
                else:
                    st.error("No se encontraron videos válidos en ninguno de los canales proporcionados.")
        else:
            st.warning("Introduce al menos un canal o usuario.")
    ...
    if not st.session_state.db_fallidos.empty:
        with st.expander("❌ REPORTE DE ERRORES (CANALES Y LINKS FALLIDOS)"):
            st.table(st.session_state.db_fallidos)
